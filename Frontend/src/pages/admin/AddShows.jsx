...
const AddShows = () => {
  ...
  useEffect(() => {
    ...
    const fetchNowPlaying = async () => {
      ...
      try {
        ...
        console.error(err);
        if (isMounted) toast.error("Failed to fetch movies");
      } finally {
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      console.error(err);
      toast.error("Error adding show. Try again.");
    } finally {
      setAddingShow(false);
    }
  };
      ...
      <div className="overflow-x-auto pb-4">
        <div className="flex flex-wrap gap-4 mt-4 w-max">
          {nowPlayingMovies.map((movie) => (
            <div
              key={movie.id}
              className="relative max-w-40 cursor-pointer hover:opacity-80 transition duration-300"
