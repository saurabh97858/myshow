...
const handleRemoveTime = (date,time)=> {
  setdateTimeSelection((prev)=>{
    const filteredTimes = prev [date].filter((t)=> t !== time);if(filteredTimes.length === 0){
...
{nowPlayingMovies.map((movie)=>(
  <div key={movie.id} className={`relative max-w-40 cursor-pointer group-hover:not-hover:opacity-40 hover:-transition-y-1 transition duration-300`} onClick={()=>setSelectedMovie(movie.id)}>
...
{/* show price input */}
<div className='mt-8'>
<label className='block text-sm font-mediu mb-2'>Show Price</label>
...
<div className='mt-6'>
<label className='block text-sm font-mediu mb-2'>Select Date & Time</label>
