...
const SeatLayout = () => {
  ...
  const getShow = async () => {
    ...
    const foundShow = dummyShowsData.find((s) => s.id === showId);

    if (foundShow) {
      setShow({
        movie: foundShow,
        dateTime: dummyDateTimeData[showId] || [],
      });
    }
  };
...
<button onClick={()=>navigate ('/my-bookings')} className='flex flex-items gap-1 mt-20 px-10 py-3 text-sm bg-primary hover:bg-primary-dull transition rounded-full font-medium cursor-pointer active:scale-95'>Procceed to check out 
